...
public class ListviewController<T> where T : IDisplayableInformation
{
    ...
    public ListView InitializeItemList(VisualElement root, VisualTreeAsset listElementTemplate, List<T> source, Action<IEnumerable<object>> notifyController)
    {
        ...
        // When using ScrollingMenu.uxml
        if(m_MenuList is null)
            m_MenuList = root.Q<ListView>("TransparentListView");
    ...
    void FillItemList()
    {
        ...
        m_MenuList.makeItem = () =>
        {
            ...
            var newListEntryLogic = new ListviewEntryController<T>();
        ...
        {
            (item.userData as ListviewEntryController<T>).SetItemData(m_SourceData[index]);
        };
